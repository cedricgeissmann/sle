...
export function splitHLBytes(byte) {
  if (typeof byte !== "string") throw new Error("byte has to be a string of 2 characters")
  return byte.split("").map(char => parseInt(char, 16))
}
...
export function hexToDec(hex) {
  if (typeof hex !== "string") throw new Error(`'${hex}' has to be a string of 2 characters`)
...
export function stringToHexArray(str) {
  return str.split("").map(char => {
    let c = char.charCodeAt(0).toString(16).toUpperCase()
    if (c.length === 1) return "0" + c
    return c
  })
}

export function hexStringToString(hexArray) {
  return hexArray.map(entry => {
    const dec = hexToDec(entry)
  ...
  export function expandKey(key_) {
    ...
    for (let i = keySize; i < expandedKey.length; i += 4) {
      ...
      if (i % keySize === 0) {
        ...
        temp = [
          xor(sLookup(temp[1]), RCON[rconIndex]),
          sLookup(temp[2]),
          sLookup(temp[3]),
          sLookup(temp[0]),
        ];
      ...
      if (i % keySize < 16) {
        for (let j = 0; j < 4; j++)
          expandedKey[i + j] = xor(expandedKey[i - keySize + j], temp[j])
      }
    }
